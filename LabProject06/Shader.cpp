...
void CShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dRootSignature)
{
	...
	pd3dDevice->CreateGraphicsPipelineState(
		...
		IID_PPV_ARGS(m_ppd3dPipelineStates[0].GetAddressOf())
	);
...
{
	// m_nObjects = 1;
	m_ppObjects.resize(1);

	m_ppObjects[0] = std::make_unique<CGameObject>();
	m_ppObjects[0]->SetMesh(std::make_shared<CTriangleMesh>(pd3dDevice, pd3dCommandList));
}
